...
st.markdown("""<style>body {background-color: #A8A2F;} .stApp { background-color: #A8A2F; } </style>""", unsafe_allow_html=True)
st.markdown("<h5 style='text-align: center; color: Blue';>SCRAPING SUR COINAFRIQUE AVEC BEAUTIFULSOUP ET WEB-SCRAPER</h5>", unsafe_allow_html=True) 

st.markdown(""" <style> section[data-testid="stSidebar"] {background-color: #BCC1DC; }  
...
<div class="sidebar-caption">
    <b><u>PROJET DE DATA COLLECTION</u></b> <br/>BANIEK Eudes Exaucé <br/> 
</div>
...
def scraper_Data_coinaf(nbrepage,produits):
...
def nettoyerprix(prix):
    ...
    if prix_nettoye >500000:
        return np.nan
    return prix_nettoye


if  choix == 'Tableau de bord':
    dtfrm_chaus_enf = pd.read_csv('Chaussures_Enfant.csv', encoding='ISO-8859-1', sep=';', on_bad_lines='skip')
    dtfrm_chaus_hom = pd.read_csv('Chaussures_Homme.csv', encoding='ISO-8859-1', sep=';', on_bad_lines='skip')
    dtfrm_vetem_enf = pd.read_csv('Vetements_Enfant.csv', encoding='ISO-8859-1', sep=';', on_bad_lines='skip')
    dtfrm_vetem_hom = pd.read_csv('Vetements_Homme.csv', encoding='ISO-8859-1', sep=';', on_bad_lines='skip') 
   ...
   #Graph
    ...
    with st.spinner('Scraping en cours...'):
        dtfrm = scraper_Data_coinaf(nbre_pages,'chaussures-enfants') 
    ...
    components.html("""<iframe src="https://ee.kobotoolbox.org/x/aHTKbU2G" width="700" height="1000"</iframe>""",height=1150,width=900)
